...
public class ticket extends AppCompatActivity {
    ...
    private EditText etDepartureCity, etArrivalCity, etDate, etClass;
    private RecyclerView recyclerView;
    private TicketAdapter ticketAdapter;
    private List<Pair<Tickets, Flight>> filteredTickets = new ArrayList<>();

    private DatabaseReference flightsRef = FirebaseDatabase.getInstance().getReference("flights");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ticketAdapter = new TicketAdapter(filteredTickets);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        recyclerView.setAdapter(ticketAdapter);

        findViewById(R.id.findTicketButton).setOnClickListener(v -> searchTickets());
    ...
    private void searchTickets() {
        String depCity = etDepartureCity.getText().toString().trim();
        String arrCity = etArrivalCity.getText().toString().trim();
        String date = etDate.getText().toString().trim(); // формат: дд/мм/гггг
        String ticketClass = etClass.getText().toString().trim();

        if (TextUtils.isEmpty(depCity) || TextUtils.isEmpty(arrCity) || TextUtils.isEmpty(date) || TextUtils.isEmpty(ticketClass)) {
        ...
        flightsRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot flightsSnapshot) {
                ...
                for (DataSnapshot flightSnapshot : flightsSnapshot.getChildren()) {
                    ...
                    if (flight != null &&
                            flight.getDepartureCity().equalsIgnoreCase(depCity) &&
                            flight.getArrivalCity().equalsIgnoreCase(arrCity)) {

                        String flightDate = flight.getDepartureTime().split(" ")[0];
                ...
                loadTickets(matchingFlights, ticketClass);
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(ticket.this, "Ошибка загрузки рейсов", Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void loadTickets(Map<String, Flight> flightsMap, String ticketClass) {
        ticketsRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot ticketSnapshot) {
                ...
                for (DataSnapshot snap : ticketSnapshot.getChildren()) {
                    Tickets ticket = snap.getValue(Tickets.class);
                    if (ticket != null && flightsMap.containsKey(ticket.getFlightId())
                            && ticket.getTicketClass().equalsIgnoreCase(ticketClass)) {

                        filteredTickets.add(new Pair<>(ticket, flightsMap.get(ticket.getFlightId())));
                    }
                }
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(ticket.this, "Ошибка загрузки билетов", Toast.LENGTH_SHORT).show();
            }
        });
    }
